window.IDJLT_PHRASES = [
  {
    id: "lesson6-phrase-1",
    ru: "Я читаю книгу.",
    en: "I read a book.",
    jp: "私は 本を よみます。",
    romaji: "Watashi wa hon o yomimasu."
  },
  {
    id: "lesson6-phrase-2",
    ru: "Я покупаю газеты на станции.",
    en: "I buy newspapers at the station.",
    jp: "私は えきで しんぶんを かいます。",
    romaji: "Watashi wa eki de shinbun o kaimasu."
  },
  {
    id: "lesson6-phrase-3",
    ru: "Вы не поедете с нами в Кобе?",
    en: "Won't you go to Kobe with us?",
    jp: "いっしょに こうべへ いきませんか。",
    romaji: "Issho ni Koube e ikimasen ka."
  },
  {
    id: "lesson6-phrase-4",
    ru: "Немного отдохнём.",
    en: "Let's take a short rest.",
    jp: "ちょっと やすみましょう。",
    romaji: "Chotto yasumimashou."
  },
  {
    id: "lesson6-phrase-5",
    ru: "Завтра давайте встретимся на станции в 10 часов.\n— Понятно.",
    en: "Let's meet at the station tomorrow at 10 o'clock.\n— Understood.",
    jp: "あした じゅうじに えきで あいましょう。\n— わかりました。",
    romaji: "Ashita juuji ni eki de aimashou.\n— Wakarimashita."
  },
  {
    id: "lesson6-phrase-6",
    ru: "Вы пьёте спиртное?\n— Нет, не пью.",
    en: "Do you drink alcohol?\n— No, I don't.",
    jp: "おさけを のみますか。\n— いいえ、のみません。",
    romaji: "Osake o nomimasu ka.\n— iie, nomimasen."
  },
  {
    id: "lesson6-phrase-7",
    ru: "Что вы едите каждое утро?\n— Ем хлеб и яйцо.",
    en: "What do you eat every morning?\n— I eat bread and an egg.",
    jp: "まいあさ なにを たべますか。\n— パンと 卵を たべます。",
    romaji: "Maiasa nani o tabemasu ka.\n— Pan to tamago o tabemasu."
  },
  {
    id: "lesson6-phrase-8",
    ru: "Что вы ели сегодня утром?\n— Ничего не ел.",
    en: "What did you eat this morning?\n— I didn't eat anything.",
    jp: "けさ なにを たべましたか。\n— なにも たべませんでした。",
    romaji: "Kesa nani o tabemashita ka.\n— Nani mo tabemasen deshita."
  },
  {
    id: "lesson6-phrase-9",
    ru: "Что вы делали в субботу?\n— Занимался японским. Потом вместе с другом смотрел фильм.",
    en: "What did you do on Saturday?\n— I studied Japanese. After that I watched a movie with a friend.",
    jp: "どようび なにをしましたか。\n— 日本語を べんきょうしました。それから 友だちと えいがをみました。",
    romaji: "Doyoubi nani o shimashita ka.\n— Nihongo o benkyou shimashita. Sorekara tomodachi to eiga o mimashita."
  },
  {
    id: "lesson6-phrase-10",
    ru: "Где вы купили этот портфель?\n— В Мексике.",
    en: "Where did you buy that bag?\n— In Mexico.",
    jp: "どこで そのかばんを かいましたか。\n— メキシコで かいました。",
    romaji: "Doko de sono kaban o kaimashita ka.\n— Mekishiko de kaimashita."
  },
  {
    id: "lesson6-phrase-11",
    ru: "Завтра не поиграем ли в теннис?\n— Да, с удовольствием.",
    en: "Shall we play tennis tomorrow?\n— Yes, that sounds good.",
    jp: "あした テニスを しませんか。\n— ええ、いいですね。",
    romaji: "Ashita tenisu o shimasen ka.\n— Ee, ii desu ne."
  },
  {
    id: "lesson7-phrase-1",
    ru: "Я смотрю фильмы на компьютере.",
    en: "I watch movies on a computer.",
    jp: "私は パソコンで えいがを みます。",
    romaji: "Watashi wa pasokon de eiga o mimasu."
  },
  {
    id: "lesson7-phrase-2",
    ru: "Я преподнесу (преподношу) цветы госпоже Кимура.",
    en: "I will give flowers to Ms. Kimura.",
    jp: "私は きむらさんに はなを あげます。",
    romaji: "Watashi wa Kimura-san ni hana o agemasu."
  },
  {
    id: "lesson7-phrase-3",
    ru: "Я получил шоколад от госпожи Карины.",
    en: "I received chocolate from Ms. Karina.",
    jp: "私は カリナさんに チョコレートを もらいました。",
    romaji: "Watashi wa Karina-san ni chokoreeto o moraimashita."
  },
  {
    id: "lesson7-phrase-4",
    ru: "Я уже отправил письмо.",
    en: "I have already sent the email.",
    jp: "私は もう メールを おくりました。",
    romaji: "Watashi wa mou meeru o okurimashita."
  },
  {
    id: "lesson7-phrase-5",
    ru: "Вы занимались японским по телевизору?\n— Нет, по радио.",
    en: "Did you study Japanese using television?\n— No, using the radio.",
    jp: "テレビで 日本語を べんきょうしましたか。\n— いいえ、ラジオで べんきょうしました。",
    romaji: "Terebi de nihongo o benkyou shimashita ka.\n— iie, rajio de benkyou shimashita."
  },
  {
    id: "lesson7-phrase-6",
    ru: "Вы пишете доклад по-японски?\n— Нет, по-английски.",
    en: "Will you write the report in Japanese?\n— No, in English.",
    jp: "日本語で レポートを 書きますか。\n— いいえ、えいごで 書きます。",
    romaji: "Nihongo de repooto o kakimasu ka.\n— iie, eigo de kakimasu."
  },
  {
    id: "lesson7-phrase-7",
    ru: "Как по-японски «goodbye»?\n— «Саёнара».",
    en: "How do you say “goodbye” in Japanese?\n— “Sayounara.”",
    jp: "“Good bye”は 日本語で なんですか。\n— 「さようなら」です。",
    romaji: "“Good bye” wa nihongo de nan desu ka.\n— “Sayounara” desu."
  },
  {
    id: "lesson7-phrase-8",
    ru: "Кому вы пишете новогодние открытки?\n— Преподавателю и друзьям.",
    en: "Who will you write New Year cards to?\n— My teacher and friends.",
    jp: "だれに ねんがじょうを 書きますか。\n— 先生と 友だちに 書きます。",
    romaji: "Dare ni nengajou o kakimasu ka.\n— Sensei to tomodachi ni kakimasu."
  },
  {
    id: "lesson7-phrase-9",
    ru: "Что это у вас?\n— Записная книжка. Получил в подарок от господина Ямада.",
    en: "What is that?\n— It is a notebook. I received it from Mr. Yamada.",
    jp: "それは なんですか。\n— 手ちょうです。山田さんに もらいました。",
    romaji: "Sore wa nan desu ka.\n— Techou desu. Yamada-san ni moraimashita."
  },
  {
    id: "lesson7-phrase-10",
    ru: "Вы уже купили билеты на синкансэн?\n— Да, уже купил.",
    en: "Have you bought the Shinkansen tickets yet?\n— Yes, I have.",
    jp: "もう しんかんせんの きっぷを かいましたか。\n— はい、もう かいました。",
    romaji: "Mou shinkansen no kippu o kaimashita ka.\n— Hai, mou kaimashita."
  },
  {
    id: "lesson7-phrase-11",
    ru: "Вы уже пообедали?\n— Нет ещё. Сейчас собираюсь поесть.",
    en: "Have you had lunch yet?\n— Not yet. I am going to eat now.",
    jp: "もう ひるごはんを たべましたか。\n— いいえ、まだです。これから たべます。",
    romaji: "Mou hirugohan o tabemashita ka.\n— iie, mada desu. Kore kara tabemasu."
  },
  {
    id: "lesson7-v2-phrase-1",
    setId: "lesson7-v2",
    setTitle: { ru: "Урок 7 — Ver. 2", en: "Lesson 7 — Ver. 2" },
    ru: "В России не едят палочками. Используют вилку, ложку и нож.",
    en: "In Russia, people do not eat with chopsticks. They use a fork, spoon, and knife.",
    jp: "ロシアで はしで たべません。\nフォークと スプーンと ナイフを つかいます。",
    romaji: "Roshia de hashi de tabemasen.\nFooku to supuun to naifu o tsukaimasu."
  },
  {
    id: "lesson7-v2-phrase-2",
    setId: "lesson7-v2",
    setTitle: { ru: "Урок 7 — Ver. 2", en: "Lesson 7 — Ver. 2" },
    ru: "Я сфотографировал кошку на мобильный телефон сестры.",
    en: "I took a photo of a cat with my older sister's mobile phone.",
    jp: "私は あねのケータイで ねこのしゃしんを とりました。",
    romaji: "Watashi wa ane no keetai de neko no shashin o torimashita."
  },
  {
    id: "lesson7-v2-phrase-3",
    setId: "lesson7-v2",
    setTitle: { ru: "Урок 7 — Ver. 2", en: "Lesson 7 — Ver. 2" },
    ru: "Сколько часов лететь до Токио на самолёте?",
    en: "How many hours does it take to fly to Tokyo?",
    jp: "とうきょうまで ひこうきで なんじかんですか。",
    romaji: "Toukyou made hikouki de nanjikan desu ka."
  },
  {
    id: "lesson7-v2-phrase-4",
    setId: "lesson7-v2",
    setTitle: { ru: "Урок 7 — Ver. 2", en: "Lesson 7 — Ver. 2" },
    ru: "Как сказать «степлер» по-японски?",
    en: "How do you say 'stapler' in Japanese?",
    jp: "「степлер」は 日本語で なんですか。",
    romaji: "'Stepler' wa nihongo de nan desu ka."
  },
  {
    id: "lesson7-v2-phrase-5",
    setId: "lesson7-v2",
    setTitle: { ru: "Урок 7 — Ver. 2", en: "Lesson 7 — Ver. 2" },
    ru: "Как будет «ластик» и «дырокол» по-испански?",
    en: "How do you say 'eraser' and 'hole punch' in Spanish?",
    jp: "「ластик」と「дырокол」は スペイン語で なんですか。",
    romaji: "'Lastik' to 'dyrokol' wa supeingo de nan desu ka."
  },
  {
    id: "lesson7-v2-phrase-6",
    setId: "lesson7-v2",
    setTitle: { ru: "Урок 7 — Ver. 2", en: "Lesson 7 — Ver. 2" },
    ru: "Как сказать «спасибо» по-китайски?",
    en: "How do you say 'thank you' in Chinese?",
    jp: "「спасибо」は ちゅうごく語で なんですか。",
    romaji: "'Spasibo' wa chuugokugo de nan desu ka."
  },
  {
    id: "lesson7-v2-phrase-7",
    setId: "lesson7-v2",
    setTitle: { ru: "Урок 7 — Ver. 2", en: "Lesson 7 — Ver. 2" },
    ru: "Что ты в прошлом году подарил отцу на день рождения?\n— Я подарил открытку и шарф.",
    en: "What did you give your father for his birthday last year?\n— I gave him a postcard and a scarf.",
    jp: "きょねん 父のたんじょうびに なにを あげましたか。\n— 私は はがきと マフラーを あげました。",
    romaji: "Kyonen chichi no tanjoubi ni nani o agemashita ka.\n— Watashi wa hagaki to mafuraa o agemashita."
  },
  {
    id: "lesson7-v2-phrase-8",
    setId: "lesson7-v2",
    setTitle: { ru: "Урок 7 — Ver. 2", en: "Lesson 7 — Ver. 2" },
    ru: "Кто дал тебе эту книгу на итальянском языке?\n— Я получил её от учителя.",
    en: "Who gave you this Italian-language book?\n— I received it from my teacher.",
    jp: "だれが あなたに このイタリア語のほんを あげましたか。\nあなたは だれに このイタリア語のほんを もらいましたか。\n— 先生に もらいました。",
    romaji: "Dare ga anata ni kono itariago no hon o agemashita ka.\nAnata wa dare ni kono itariago no hon o moraimashita ka.\n— Sensei ni moraimashita."
  },
  {
    id: "lesson7-v2-phrase-9",
    setId: "lesson7-v2",
    setTitle: { ru: "Урок 7 — Ver. 2", en: "Lesson 7 — Ver. 2" },
    ru: "Этот галстук и рубашку я получил от мамы.",
    en: "I received this tie and shirt from my mother.",
    jp: "私は ははに このネクタイとシャツを もらいました。",
    romaji: "Watashi wa haha ni kono nekutai to shatsu o moraimashita."
  },
  {
    id: "lesson7-v2-phrase-10",
    setId: "lesson7-v2",
    setTitle: { ru: "Урок 7 — Ver. 2", en: "Lesson 7 — Ver. 2" },
    ru: "Я подарил младшему брату новый смартфон.",
    en: "I gave my younger brother a new smartphone.",
    jp: "私は おとうとに あたらしいスマホを あげました。",
    romaji: "Watashi wa otouto ni atarashii sumaho o agemashita."
  },
  {
    id: "lesson8-phrase-1",
    ru: "Сакура красива.",
    en: "Cherry blossoms are beautiful.",
    jp: "さくらは きれいです。",
    romaji: "Sakura wa kirei desu."
  },
  {
    id: "lesson8-phrase-2",
    ru: "Гора Фудзи высокая.",
    en: "Mount Fuji is high.",
    jp: "ふじさんは たかい やま です。",
    romaji: "Fujisan wa takai yama desu."
  },
  {
    id: "lesson8-phrase-3",
    ru: "Сакура — красивый цветок.",
    en: "Cherry blossom is a beautiful flower.",
    jp: "さくらは きれいな はな です。",
    romaji: "Sakura wa kirei na hana desu."
  },
  {
    id: "lesson8-phrase-4",
    ru: "Гора Фудзи — высокая гора.",
    en: "Mount Fuji is a high mountain.",
    jp: "ふじさんは たかい やまです。",
    romaji: "Fujisan wa takai yama desu."
  },
  {
    id: "lesson8-phrase-5",
    ru: "В Осака оживлённо?— Да, оживлённо.",
    en: "Is Osaka lively? - Yes, it is lively.",
    jp: "おおさかは にぎやかですか。\nはい、にぎやかです。",
    romaji: "Oosaka wa nigiyaka desu ka.\nHai, nigiyaka desu."
  },
  {
    id: "lesson8-phrase-6",
    ru: "Университет Сакура известный (университет)?\n- Нет, не очень (известный).",
    en: "Is Sakura University famous? - No, not very famous.",
    jp: "さくらだいがくは ゆうめいですか。\nいいえ、ゆうめいじゃありません。",
    romaji: "Sakura daigaku wa yuumei desu ka.\nIie, yuumei ja arimasen."
  },
  {
    id: "lesson8-phrase-7",
    ru: "В Пекине сейчас холодно?— Да, очень холодно.",
    en: "Is it cold in Beijing now? - Yes, very cold.",
    jp: "ペキンは いま さむいですか。\nいいえ、あまり さむくないです。",
    romaji: "Pekin wa ima samui desu ka.\nIie, amari samukunai desu."
  },
  {
    id: "lesson8-phrase-8",
    ru: "В Шанхае тоже холодно?\n- Нет, не очень холодно.",
    en: "Is it cold in Shanghai too? - No, not very cold.",
    jp: "シャンハイも さむいですか。\nいいえ、あまり さむくないです。",
    romaji: "Shanhai mo samui desu ka.\nIie, amari samukunai desu."
  },
  {
    id: "lesson8-phrase-9",
    ru: "Как вам университетское общежитие?\n- Хотя старое, но удобное.",
    en: "How is the university dormitory? - It is old, but convenient.",
    jp: "だいがくの りょうは どうですか。\nふるいですが、べんりです。",
    romaji: "Daigaku no ryou wa dou desu ka.\nFurui desu ga, benri desu."
  },
  {
    id: "lesson8-phrase-10",
    ru: "Вчера ходил домой к Мацумото сан.\n- Какой (у него) дом?\nКрасивый дом. И большой.",
    en: "Yesterday I went to Mr. Matsumoto's house. - What kind of house is it? A beautiful house. And big.",
    jp: "きのう まつもとさんの うちへ いきました。\nどんなうちですか。\nきれいな うちです。 そして、おおきい うちです。",
    romaji: "Kinou Matsumoto-san no uchi e ikimashita.\nDonna uchi desu ka.\nKirei na uchi desu. Soshite, ookii uchi desu."
  },
  {
    id: "lesson8-phrase-11",
    ru: "Вчера посмотрел интересный фильм.\n- Какой (фильм)?\n«Семь самураев».",
    en: "Yesterday I watched an interesting movie. - Which movie? Seven Samurai.",
    jp: "きのう おもしろい えいがを みました。\nなにを みましたか。\n「しちにんの さむらい」です。",
    romaji: "Kinou omoshiroi eiga o mimashita.\nNani o mimashita ka.\nShichinin no samurai desu."
  },
  {
    id: "lesson8-phrase-12",
    ru: "Утром шел сильный дождь. На дорогах образовались глубокие лужи (みずたまりができました).",
    en: "In the morning it rained heavily. Deep puddles formed on the roads.",
    jp: "今朝、大雨が 降りました。道に 大きな 水たまり\nができました。",
    romaji: "Kesa ooame ga furimashita. Michi ni ookina mizutamari ga dekimashita."
  },
  {
    id: "lesson8-phrase-13",
    ru: "Здания в Санкт-Петербурге очень красивые.\nИ к тому же известные.",
    en: "The buildings in Saint Petersburg are very beautiful. And famous too.",
    jp: "サンクト・ペテルブルグの建物は とてもきれいです。 そして、ゆうめいです。",
    romaji: "Sankuto Peteruburugu no tatemono wa totemo kirei desu. Soshite, yuumei desu."
  },
  {
    id: "lesson8-phrase-14",
    ru: "Вчера вечером с женой обедал в дорогом японском ресторане.",
    en: "Yesterday evening I had lunch with my wife at an expensive Japanese restaurant.",
    jp: "きのうの夜、妻と 高級日本食料理レストランで昼ご\nはんを たべました。",
    romaji: "Kinou no yoru, tsuma to koukyuu nihonshoku ryouri resutoran de hirugohan o tabemashita."
  },
  {
    id: "lesson8-phrase-15",
    ru: "В этом озере вода не очень чистая.",
    en: "The water in this lake is not very clean.",
    jp: "この湖の 水は あまり きれいじゃありません。",
    romaji: "Kono mizuumi no mizu wa amari kirei ja arimasen."
  },
  {
    id: "lesson8-phrase-16",
    ru: "Климат （きこう） в Испании очень жаркий.",
    en: "The climate in Spain is very hot.",
    jp: "スペインの気候は とても 暑いです。",
    romaji: "Supein no kikou wa totemo atsui desu."
  },
  {
    id: "lesson8-phrase-17",
    ru: "В прошлом году сделал много красивых фотографий озера «Байкал».",
    en: "Last year I took many beautiful photos of Lake Baikal.",
    jp: "去年 きれいなバイカル湖の写真をたくさん撮りました。",
    romaji: "Kyonen kirei na Baikaru-ko no shashin o takusan torimashita."
  },
  {
    id: "lesson8-phrase-18",
    ru: "Сегодня он себя плохо чувствует.",
    en: "Today he is not feeling well.",
    jp: "今日、あの人は 体調が 良くないです。",
    romaji: "Kyou, ano hito wa taichou ga yokunai desu."
  },
  {
    id: "lesson8-phrase-19",
    ru: "На прошлой неделе в Московском зоопарке （どうぶつえん） видел\nмного редких （めずらしい） зверей и птиц.",
    en: "Last week at the Moscow Zoo I saw many rare animals and birds.",
    jp: "先週、モスクワ動物園で 珍しい動物 と 鳥を見ま\nした。",
    romaji: "Senshuu Mosukuwa doubutsuen de mezurashii doubutsu to tori o mimashita."
  },
  {
    id: "lesson8-phrase-20",
    ru: "В последнее время она очень занята.",
    en: "Recently she has been very busy.",
    jp: "最近、あの人は とても 忙しいです。",
    romaji: "Saikin, ano hito wa totemo isogashii desu."
  },
  {
    id: "lesson8-phrase-21",
    ru: "Этот компьютер очень дорогой, да и не очень удобный.",
    en: "This computer is very expensive, and not very convenient.",
    jp: "このコンピューターは とても高いですが、あまり\n便利じゃありません。\n\n（このコンピューターはすごく高いのに、使い勝手が\nあまり よくないです。）",
    romaji: "Kono konpyuutaa wa totemo takai desu ga, amari benri ja arimasen.\nKono konpyuutaa wa sugoku takai noni, tsukaigatte ga amari yokunai desu."
  }];
